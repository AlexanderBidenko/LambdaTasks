...
export class CryptocurrencyApi { 
	...
	public async hypeCurrencysAverage(period=30) {  
		...
		new Promise( () => { 
			res.forEach(element => {
				...
				const someCoinData = [this.date, period, element.position, element.symbol, AverageCoinbase , AverageCoinstats , AverageKucoin , AverageCoinpaprika, element.totalAverage];
				...
				// const someCoinData = [this.date, period, element.position, element.symbol, element.marketsAverage.coinmarketcap , element.marketsAverage.coinbase , element.marketsAverage.coinstats , element.marketsAverage.kucoin , element.marketsAverage.coinpaprika, element.totalAverage]
				const sql = `INSERT INTO  averageValues${period} (time, period, position, symbol, CoinBase, CoinStats, Kucoin, CoinPaprika, totalAverage) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`;
	...
	public  async updateAll(){
		...
		console.log("Updating cryptocurrency average prices");
		await Promise.all([
		...
		]).then(()=>console.log("Cryptocurrency average prices had updated")).finally(() => {
			this.connection.end();
		});
	}
