...
function appendUser() {
  ...
  inquirer.prompt([
  {
    ...
    validate(userName) {
      ...
      //можно при желании раскомитить и будет валидация уникального юзернейма
      // let db = './db.txt';
      // const connectingToDB = fs.createReadStream(db);
      // let resValidationSearch = searchUserCB(userName, connectingToDB)
      // resValidationSearch.then(result => {
      //   if (result.size > 0) {
